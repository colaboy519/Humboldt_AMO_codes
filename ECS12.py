...
def plotEnergy_numerical_analytical(parameters, m_show=0, parity_show="", n_start_value = 5, n_end_value = 10):
    ...
    ax.plot(dummy_x_axi, numerical_energies,'o', label=f"{i}th numerical energy ")
...
def main(): 
    ...
    parser = argparse.ArgumentParser(description=\
                                    'This python script can separately create input file for FORTRAN procedure,\
                                     feed input file into running FORTRAN procedure,\
                                     and read output from FORTRAN procedure')
    
    # Add the arguments
    parser.add_argument('-c','--create',
                        action='store_true',
                        help='Create the tempate text file to be fed into FORTRAN code.')
    parser.add_argument('-r','--run',
                        action='store_true',
                        help='Feed the input file into FORTRAN program for calculation.')
    parser.add_argument('-s','--show',
                        action='store_true',
                        help='Read the output file from Fortran Programs (unformatted)\
                                and print the first 5 distinct energy levels\
                                    (consider 1st value for each degenerate energy level)')
    parser.add_argument('-pe','--plot_energy',
                        action='store_true',
                        help='Plot numerical and analytical energy levels for a specified range on the same plot.')
    parser.add_argument('-pw','--plot_wavefunction',
                        action='store_true',
                        help='Read the FORTRAN output of the parameters (unformatted) and plot the wavefunction.')
    ...
    parameters = system_basis_parameters
    # print currnent system_basis_parameters
    print('Expected states number:', parameters['N_x'] * parameters['N_y'] - parameters['k_x'] - parameters['k_y'] + 3)
    print("current parameters: ", parameters)

    # for readubg and plotting output
    m_show_ = 0
    parity_show_ = "g"

    # run the programs
    if args.create: 
        createInputFile(parameters)
    if args.run: 
        runCalculation(parameters)
    if args.show:
        EnT = readEnergy(parameters,m_show=m_show_,parity_show=parity_show_)
        print("Number of states calculated: ", len(EnT))
        energies = get_energy_levels(EnT)
        for n in range(0,5): 
            print(energies[n])
        print("finished task s")
    if args.plot_energy:
        plotEnergy_numerical_analytical(parameters, m_show=m_show_, parity_show=parity_show_)
    if args.plot_wavefunction:
        plotWaveFunctions(parameters, m_show=m_show_, parity_show=parity_show_)
    if not (args.create or args.run or args.show or args.plot_energy or args.plot_wavefunction):
        print("Please provide command line parameters: '-c', '-r', '-s', '-pe', '-pw'")
